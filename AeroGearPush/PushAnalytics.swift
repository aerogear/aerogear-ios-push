    ...
    */
    class open func sendMetricsWhenAppLaunched(launchOptions: [AnyHashable: Any]?, completionHandler: @escaping ((_ error: NSError? ) -> Void) = {(error: NSError?) in }) {
        if let options = launchOptions {
            if let option : NSDictionary = options[UIApplication.LaunchOptionsKey.remoteNotification] as? NSDictionary {
    ...
    :param: completionHandler A block object to be executed when the send metrics operation finishes. Defaulted to no action.
    */
    class open func sendMetricsWhenAppAwoken(applicationState: UIApplication.State, userInfo: [AnyHashable: Any], completionHandler: @escaping ((_ error: NSError? ) -> Void) = {(error: NSError?) in }) {
